...
nem <- read.delim('1_arable_nem_table.txt', row.name = 1, check.names = FALSE)
dim(nem)
...
diag(z) <- 0
head(z)[1:6,1:6]
write.table(data.frame(z, check.names = FALSE), '2_arable_nem_corr.matrix.txt',
...
write.table(data.frame(adj_matrix, check.names = FALSE), '3_arable_nem_network.adj_matrix.txt', col.names = NA, sep = '\t', quote = FALSE)##保存邻接矩阵
...
setwd(readClipboard())
getwd()
initial_adjacency<-read.delim('1_arable_nem_network.adj_matrix.txt',
...
initial_abundance<-read.delim('2_arable_nem_abundance.txt',row.names=1,sep='\t',check.names=FALSE)#
...
head(sub_graph_stat)
write.csv(sub_graph_stat, 'Arable_nem_subnetwork_indices.csv', quote = FALSE)
...
nem <- read.delim('arable6-1-abundance.txt', row.name = 1, check.names = FALSE)
nem <- t(nem)
...
dat1<-data.frame(nemportion.removed=rep(seq(0.05,1,by=0.05),2),rbind(Weighted.simu,Unweighted.simu),
                 ...
                 biota=rep("arable-nem",40),treat=rep("arable",40))
currentdat<-dat1
currentdat[1:10,1:7]
write.csv(currentdat,"6-1-arable_nem_random_removal_result.csv")
