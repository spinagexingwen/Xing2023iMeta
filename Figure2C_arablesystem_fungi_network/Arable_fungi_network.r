...
fungi <- read.delim('1_arable_fun_asv_table.txt', row.name = 1, check.names = FALSE)
dim(fungi)
fungi <- fungi[which(rowMeans(fungi) > 0.0001), ]
dim(fungi)
fungi <- t(fungi)

#######Correlation analysis########
fungi_corr <- rcorr(fungi, type = 'spearman')
r <- fungi_corr$r
r[abs(r) < 0.6] <- 0
p <- fungi_corr$P
...
diag(z) <- 0
head(z)[1:6,1:6]
write.table(data.frame(z, check.names = FALSE), '2_arable_fun_corr.matrix.txt',
...
write.table(data.frame(adj_matrix, check.names = FALSE), '3_arable_fun_network.adj_matrix.txt', col.names = NA, sep = '\t', quote = FALSE)
...
setwd(readClipboard())
getwd()
initial_adjacency<-read.delim('1_arable_fun_network.adj_matrix.txt',
...
initial_abundance<-read.delim('2_arable_fun_abundance.txt',row.names=1,sep='\t',check.names=FALSE)
...
head(sub_graph_stat)
write.csv(sub_graph_stat, 'Arable_fungi_subnetwork_indices.csv', quote = FALSE)
...
fungi <- read.delim('arable6-1abundance.txt', row.name = 1, check.names = FALSE)
fungi <- t(fungi)
genus_corr <- rcorr(fungi, type = 'spearman')
r <- genus_corr$r
r[abs(r) < 0.60] <- 0
p <- genus_corr$P
#p <- p.adjust(p, method = 'BH')
...
network.raw[1:3,1:3]
otutab<-read.table("arable6-1abundance.txt",header = T,row.names=1,sep="\t")
comm<-t(otutab)
sp.ra<-colMeans(comm)
head(sp.ra)
...
dat1<-data.frame(Proportion.removed=rep(seq(0.05,1,by=0.05),2),rbind(Weighted.simu,Unweighted.simu),
                 ...
                 biota=rep("arable-fungi",40),treat=rep("arable",40))
currentdat<-dat1
currentdat[1:10,1:7]
write.csv(currentdat,"6-1-arable_fun_random_removal_result.csv")
