...
bacteria <- read.delim('1_arable_bac_table.txt', row.name = 1, check.names = FALSE)
dim(bacteria)
bacteria <- bacteria[which(rowMeans(bacteria) > 0.0001), ]
dim(bacteria)
bacteria <- t(bacteria)

#######Correlation analysis########
bacteria_corr <- rcorr(bacteria, type = 'spearman')
r <- bacteria_corr$r
r[abs(r) < 0.6] <- 0
p <- bacteria_corr$P
...
diag(z) <- 0
head(z)[1:6,1:6]
write.table(data.frame(z, check.names = FALSE), '2_arable_bacteria_corr.matrix.txt',
...
write.table(data.frame(adj_matrix, check.names = FALSE), '3_arable_bac_network.adj_matrix.txt', col.names = NA, sep = '\t', quote = FALSE)##保存邻接矩阵
...
setwd(readClipboard())
getwd()
initial_adjacency<-read.delim('1_arable_bac_network.adj_matrix.txt',
...
initial_abundance<-read.delim('2_arable_bac_abundance.txt',row.names=1,sep='\t',check.names=FALSE)
...
head(sub_graph_stat)
write.csv(sub_graph_stat, 'Arable_bacteria_subnetwork_indices.csv', quote = FALSE)

adj_matrix <- as.matrix(get.adjacency(igraph))
write.table(data.frame(adj_matrix, check.names = FALSE),'Subnetwork_Arable6_1.txt', col.names = NA, sep = '\t', quote = FALSE)


########prepared the initial abundance table for sample arable6-1#####
bacteria <- read.delim('arable6-1-abundance.txt', row.name = 1, check.names = FALSE)
bacteria <- t(bacteria)
genus_corr <- rcorr(bacteria, type = 'spearman')
r <- genus_corr$r
r[abs(r) < 0.60] <- 0
p <- genus_corr$P
#p <- p.adjust(p, method = 'BH')
...
dat1<-data.frame(Proportion.removed=rep(seq(0.05,1,by=0.05),2),rbind(Weighted.simu,Unweighted.simu),
                 ...
                 biota=rep("arable-bacteria",40),treat=rep("arable",40))
currentdat<-dat1
currentdat[1:10,1:7]
write.csv(currentdat,"6-1-arable_bac_random_removal_result.csv")
