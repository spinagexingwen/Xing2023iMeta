...
bacteria <- read.delim('1_restore_bac_table.txt', row.name = 1, check.names = FALSE)
dim(bacteria)
...
diag(z) <- 0
head(z)[1:6,1:6]
write.table(data.frame(z, check.names = FALSE), '2_restore_bacteria_corr.matrix.txt',
...
write.table(data.frame(adj_matrix, check.names = FALSE), '3_restore_bac_network.adj_matrix.txt', col.names = NA, sep = '\t', quote = FALSE)
...
setwd(readClipboard())
getwd()
initial_adjacency<-read.delim('1_restore_bac_network.adj_matrix.txt',
...
initial_abundance<-read.delim('2_restore_bac_abundance.txt',row.names=1,sep='\t',check.names=FALSE)#
...
head(sub_graph_stat)
write.csv(sub_graph_stat, 'restore_bacteria_subnetwork_indices.csv', quote = FALSE)

adj_matrix <- as.matrix(get.adjacency(igraph))
write.table(data.frame(adj_matrix, check.names = FALSE),'Subnetwork_restore6_1.txt', col.names = NA, sep = '\t', quote = FALSE)


########prepared the initial abundance table for sample restore6-1#####

bacteria <- read.delim('restore6-1abundance.txt', row.name = 1, check.names = FALSE)
bacteria <- t(bacteria)
...
network.raw[1:3,1:3]
otutab<-read.table("restore6-1abundance.txt",header = T,row.names=1,sep="\t")
comm<-t(otutab)
sp.ra<-colMeans(comm)
head(sp.ra)
...
dat1<-data.frame(Proportion.removed=rep(seq(0.05,1,by=0.05),2),rbind(Weighted.simu,Unweighted.simu),
                 ...
                 biota=rep("restore-bacteria",40),treat=rep("restore",40))
currentdat<-dat1
currentdat[1:10,1:7]
write.csv(currentdat,"6-1-restore_bac_random_removal_result.csv")
