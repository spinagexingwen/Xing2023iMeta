...
whole_microbiota <- read.delim('1_arable_whole_microbiota_table.txt', row.name = 1, check.names = FALSE)
#dim(whole_microbiota)
whole_microbiota <- whole_microbiota[which(rowMeans(whole_microbiota) > 0.0001), ]
whole_microbiota <- t(whole_microbiota)

#######Correlation analysis########
whole_microbiota_corr <- rcorr(whole_microbiota, type = 'spearman')
r <- whole_microbiota_corr$r
r[abs(r) < 0.6] <- 0
p <- whole_microbiota_corr$P
...
diag(z) <- 0
head(z)[1:6,1:6]
write.table(data.frame(z, check.names = FALSE), '2_arable_whole_microbiota_corr.matrix.txt',
...
write.table(data.frame(adj_matrix, check.names = FALSE), '3_arable_whole_microbiota_network.adj_matrix.txt', col.names = NA, sep = '\t', quote = FALSE)
...
setwd(readClipboard())
getwd()
initial_adjacency<-read.delim('1_arable_whole_microbiota_network.adj_matrix.txt',
...
initial_abundance<-read.delim('2_arable_whole_microbiota_abundance.txt',row.names=1,sep='\t',check.names=FALSE)#
...
sub_graph
sub_graph$`Arable6_1`##example

plot_network(g)
plot_network(sub_graph$`Arable6_1`)
...
head(sub_graph_stat)
write.csv(sub_graph_stat, 'Arable_whole_microbiota_subnetwork_indices.csv', quote = FALSE)

adj_matrix <- as.matrix(get.adjacency(igraph))
write.table(data.frame(adj_matrix, check.names = FALSE),
            'Subnetwork_Arable6_1.txt', col.names = NA, sep = '\t', quote = FALSE)


########prepared the initial abundance table for sample arable6-1#####

whole_microbiota <- read.delim('arable6-1-abundance.txt', row.name = 1, check.names = FALSE)
whole_microbiota <- t(whole_microbiota)
genus_corr <- rcorr(whole_microbiota, type = 'spearman')
r <- genus_corr$r
r[abs(r) < 0.60] <- 0
p <- genus_corr$P
#p <- p.adjust(p, method = 'BH')
...
E(igraph)$weight <- abs(E(igraph)$weight)
adj_matrix <- as.matrix(get.adjacency(igraph, attr = 'correlation'))
write.table(data.frame(adj_matrix, check.names = FALSE), 'arable6-1-subnetwork.adj_matrix.txt', col.names = NA, sep = '\t', quote = FALSE)
...
write.table(edge_list, '!arable6-1-network.edge_list.txt',
            sep = '\t', row.names = FALSE, quote = FALSE)
######and then we calculated the numbers of positive or negative links in Excel##


##########robustness calculation########
network.raw<- as.matrix(read.delim('arable6-1-subnetwork.adj_matrix.txt', row.name = 1, check.names = FALSE))
network.raw[1:3,1:3]
otutab<-read.table("arable6-1-abundance.txt",header = T,row.names=1,sep="\t")
comm<-t(otutab)
sp.ra<-colMeans(comm)
head(sp.ra)
...
dat1<-data.frame(whole_microbiotaportion.removed=rep(seq(0.05,1,by=0.05),2),rbind(Weighted.simu,Unweighted.simu),
                 weighted=rep(c("weighted","unweighted"),each=20),
                 biota=rep("arable-whole_microbiota",40),treat=rep("arable",40))
currentdat<-dat1
currentdat[1:10,1:7]
write.csv(currentdat,"6-1-arable_whole_microbiota_random_removal_result.csv")


#######Vulnerability#########
g = graph_from_adjacency_matrix(as.matrix(read.delim('arable6-1-subnetwork.adj_matrix.txt',
...
#########keystone taxa#########
initial_adjacency<-read.delim('arable6-1-subnetwork.adj_matrix.txt',
                              row.names=1,sep='\t',check.names=FALSE)#
initial_adjacency[initial_adjacency<0]<-1
initial_adjacency[initial_adjacency>0]<-1
