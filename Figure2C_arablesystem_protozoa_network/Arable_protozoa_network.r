...
pro <- read.delim('1_arable_pro_acv_table.txt', row.name = 1, check.names = FALSE)
dim(pro)
pro <- pro[which(rowMeans(pro) > 0.0001), ]
dim(pro)
pro <- t(pro)

#######Correlation analysis########
pro_corr <- rcorr(pro, type = 'spearman')
r <- pro_corr$r
#r[abs(r) < 0.6] <- 0
p <- pro_corr$P
...
diag(z) <- 0
head(z)[1:6,1:6]
write.table(data.frame(z, check.names = FALSE), '2_arable_pro_corr.matrix.txt',
...
write.table(data.frame(adj_matrix, check.names = FALSE), '3_arable_pro_network.adj_matrix.txt', col.names = NA, sep = '\t', quote = FALSE)
...
setwd(readClipboard())
getwd()
initial_adjacency<-read.delim('1_arable_pro_network.adj_matrix.txt',
...
initial_abundance<-read.delim('2_arable_pro_abundance.txt',row.names=1,sep='\t',check.names=FALSE)#
...
head(sub_graph_stat)
write.csv(sub_graph_stat, 'Arable_pro_subnetwork_indices.csv', quote = FALSE)
...
pro <- read.delim('arable6-1-abundance.txt', row.name = 1, check.names = FALSE)
pro <- t(pro)
genus_corr <- rcorr(pro, type = 'spearman')
r <- genus_corr$r
r[abs(r) < 0.60] <- 0
p <- genus_corr$P
#p <- p.adjust(p, method = 'BH')
...
dat1<-data.frame(Proportion.removed=rep(seq(0.05,1,by=0.05),2),rbind(Weighted.simu,Unweighted.simu),
                 ...
                 biota=rep("arable-pro",40),treat=rep("arable",40))
currentdat<-dat1
currentdat[1:10,1:7]
write.csv(currentdat,"6-1-arable_pro_random_removal_result.csv")
